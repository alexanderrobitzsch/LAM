...
# cat('-- after variance_shortcut') ; zz1 <- Sys.time(); print(zz1-zz0) ; zz0 <- zz1
    ...
    if ( is.null( names( theta )) ){
        names(theta) <- paste0( 'theta', 1L:NT )
    }
    if ( is.null( names( beta )) ){
        names(beta) <- colnames(X)
        ind <- names(beta)==''
        names(beta)[ ind ] <- paste0( 'beta', 1L:NB )[ind]
    }
    #--- REML
    ...
    res <- list( id_list=id_list, G=G, y_list=y_list,
                    X_list=X_list, N=N, NB=NB, X=as.matrix(X), y=as.matrix(y),
                    id=id, NT=NT, theta=theta, Z_list=Z_list, Z_index=Z_index,
